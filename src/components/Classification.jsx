...
function Classification() {
    ...
    const newData = async () => {
        ...
        if(numero == ''){
            ...
            const response = await axios.post('http://127.0.0.1:8000/new', {'numero': numero, 'nom_class': nomClass})
    ...
    const deleteClass = (e) => {
        ...
        if(confirm_delete){
            ...
            axios.delete(`http://127.0.0.1:8000/delete/${select_data}`)
                    .then((resp) => {
                        console.log(resp);
                    })
                    .catch((err) => {
    ...
    const updateClass = (e) => {
        console.log(e.target.getAttribute('id_data'));
        setIsNew(false)
        axios.get(`http://127.0.0.1:8000/one/${e.target.getAttribute('id_data')}`)
                .then(resp => {
                    console.log(resp);
    ...
    const updateData = () => {
        axios.put(`http://127.0.0.1:8000/update/${selectData}`, {'nom_class': nomClass, 'numero': numero})
            .then(resp => {
                console.log(resp);
            })
            .catch(err => {
